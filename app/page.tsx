...
async function analyzeTextWithAI(text: string) {
  try {
    ...
    console.error("--- AI ZNÓW NIE DZIAŁA ---", errorMessage);
...
export default async function Home({ searchParams }: { searchParams: Promise<{ q?: string, mode?: string, error?: string }> }) {
  ...
  const errorMessages: Record<string, string> = {
    ...
    'email_exists': 'Ten email jest już zajęty.',
    'registration_failed': 'Wystąpił błąd rejestracji.',
  };
      ...
      <main className="flex items-center justify-center min-h-screen bg-gray-100 font-sans p-4">
        <div className="bg-white p-8 md:p-10 rounded-2xl shadow-xl w-full max-w-md text-center">
          ...
          {error && errorMessages[error] && (
            <div className="bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-lg mb-6 flex items-center gap-2 text-sm font-medium animate-pulse">
              <AlertCircle size={18} />
              {errorMessages[error]}
            </div>
          )}
          ...
          <form action={mode === 'register' ? register : login} className="flex flex-col gap-4 text-left">
            <div>
                ...
                <input name="email" type="email" placeholder="jan@przyklad.pl" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition" required />
            </div>
            <div>
                <label className="text-sm font-bold text-gray-700 mb-1 block">Hasło</label>
                <input name="password" type="password" placeholder="••••••••" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition" required />
            </div>
            
            <button className="bg-blue-600 text-white p-3 rounded-lg font-bold hover:bg-blue-700 transition mt-2 shadow-lg">
  ...
  return (
    <main className="max-w-4xl mx-auto p-6 md:p-10 font-sans">
      <div className="flex justify-between items-center mb-8">
         ...
         <form action={logout}>
             <button className="flex items-center gap-2 text-red-600 hover:bg-red-50 px-4 py-2 rounded-lg transition font-medium text-sm">
      ...
      <div className="flex flex-col items-center justify-center gap-3 mb-8">
        ...
        <form className="flex gap-2 w-full max-w-md mt-4 relative">
            <input name="q" defaultValue={query} placeholder="Szukaj notatek..." className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-full focus:ring-2 focus:ring-purple-500 focus:outline-none shadow-sm" />
        ...
        <div className="bg-white p-6 rounded-2xl shadow-md border border-gray-100 mb-10 transition hover:shadow-lg">
          <form action={addNote} className="flex flex-col gap-4">
            <div className="flex gap-4">
                ...
                <select name="color" className="p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-200 outline-none bg-white">{colorOptions.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select>
            </div>
            <textarea name="content" placeholder="O czym myślisz?" className="p-3 border border-gray-300 rounded-lg h-24 resize-none focus:ring-2 focus:ring-purple-200 outline-none" required />
            <button className="bg-purple-600 text-white py-2 px-4 rounded-lg hover:bg-purple-700 transition flex items-center gap-2 font-medium justify-center shadow-md">
            ...
            <div className="mt-auto">
                ...
                <div className="flex justify-between items-center pt-4 border-t border-gray-200/50">
                    ...
                    <div className="flex gap-3">
                        <Link href={`/edit/${note.id}`} className="text-blue-600 hover:text-blue-800 transition"><Pencil size={18}/></Link>
